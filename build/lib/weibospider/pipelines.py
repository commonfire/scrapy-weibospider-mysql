...
class WeibospiderPipeline(object):
    ...
    def _keyword_info_insert(self,conn,item,spider):
        ...
        for i in range(len(item['keyword_uid'])):
            conn.execute('''insert into cauc_keyword_info(user_id,user_alias,keyword,publish_time,content,is_delete) values(%s,%s,%s,%s,%s,%s)''',(item['keyword_uid'][i],item['keyword_alias'][i],str(item['keyword']),item['keyword_publish_time'][i],item['keyword_content'][i],0))
