...
class WeiboSpider(CrawlSpider):
    ...
    def start_getfriend_info(self,response):
        if response.status == 200:
            ...
            for round in range(1): #遍历数据库的轮数
                ...
                sql2 = "select user_id from cauc_black_man_test a \
                ...
                #更新is_search标志位为1
                sql3 = "update cauc_black_man_test set is_search = 1 where is_search = 0 and is_delete = 0"
            ...
            yield Request(url=response.request.url,cookies=random.choice(COOKIES),meta=response.meta,callback=self.start_getfriend_info)
    ...
    def parse_friend_uid(self,response):
        ...
        if response.status == 200:
            ...
            logger.warning("the failed response status code %d !!",response.status)
            #此时网络响应没有成功，重新获取cookie并请求
            yield Request(url=response.request.url,cookies=random.choice(COOKIES),meta=response.meta,callback=self.parse_friend_uid)
    ...
    def parse_userinfo(self,response):
        ...
        if response.status == 200:
            ...
            logger.warning("the failed response status code %d !!",response.status)
            #此时网络响应没有成功，重新获取cookie并请求
            yield Request(url=response.request.url,cookies=random.choice(COOKIES),meta=response.meta,callback=self.parse_userinfo)
