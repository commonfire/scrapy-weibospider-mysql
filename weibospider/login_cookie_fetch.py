...
def update_cookies(username,cookies):
    ...
    cursor.execute("insert into cauc_login_cookie_info(USERNAME,SUHB,SUB,SUBP,timestamp) values (%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE SUHB=%s,SUB=%s,SUBP=%s,timestamp=%s",(md5(username),cookies['SUHB'],cookies['SUB'],cookies['SUBP'],timestamp,cookies['SUHB'],cookies['SUB'],cookies['SUBP'],timestamp))
    conn.commit()
    db.close_connection(conn,cursor) 
...
def allcookie_fetch():
    ...
    nums = cursor.execute('SELECT SUHB,SUB,SUBP,USERNAME FROM cauc_login_cookie_info WHERE (unix_timestamp()-cast(timestamp as signed)) < (%s)',(EXPIRES,))
